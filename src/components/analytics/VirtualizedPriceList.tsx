...
const generateMockData = (count: number, currencies: string[]): PriceData[] => {
...
export const VirtualizedPriceList: React.FC = () => {
  const { currencies, timeRange, aggregation } = useSharedFilters();
  ...
  // Generate large dataset - filtered by selected currencies
  const allData = useMemo(() => {
    const data = generateMockData(10000, currencies);
    // Filter to only show selected currencies
    return data.filter(item => currencies.includes(item.symbol));
  }, [currencies]);
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <span style={{ fontSize: '10px', color: theme.textSecondary }}>
            {timeRange} • {aggregation} • {currencies.length} pairs
          </span>
